...
type User struct {
	// User's first name
	FirstName string `json:"first_name" validate:"alphaunicode,max=15,min=2"`

	// User's middle name (if exists)
	MiddleName string `json:"middle_name,omitempty" validate:"min=0,max=15,omitempty,omitnil"`

	// User's last name
	LastName string `json:"last_name" validate:"alphaunicode,max=20,min=2"`

	// User's username that is in use
	Username string `json:"username" validate:"alphanumunicode|containsrune=.-_0x20100x20110x2015,max=24,min=2"`
}
